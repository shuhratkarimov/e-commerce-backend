...
describe('CartController', () => {
  ...
  describe('create', () => {
    it('should create a new cart', async () => {
      ...
      const result = await controller.create({ cookies: { accesstoken: 'mockToken' } }, createCartDto);

      expect(result).toEqual(mockResponse);
      expect(service.create).toHaveBeenCalledWith('decodedUserId', createCartDto.products);
    });
  ...
  describe('findAll', () => {
    it('should return all carts for the user', async () => {
      ...
      const result = await controller.findOne({ cookies: { accesstoken: 'mockToken' } });
  ...
  describe('updateCartProducts', () => {
    it('should update cart products', async () => {
      const updateCartDto = { addProducts: ['product1'], removeProducts: ['product2'] };
      const mockResponse = { message: 'Cart updated!', updatedCart: {} };
      
      jest.spyOn(service, 'updateCartProducts').mockResolvedValue(mockResponse);

      const result = await controller.updateCartProducts('cartId', updateCartDto);

      expect(result).toEqual(mockResponse);
      expect(service.updateCartProducts).toHaveBeenCalledWith('cartId', updateCartDto.addProducts, updateCartDto.removeProducts);
    });
  });
