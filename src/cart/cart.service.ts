...
export class CartService {
      ...
      const carts = await this.cartModel.aggregate([
        ...
        },
        {
          $group: {
            _id: "$_id.cartId",
            products: {
              $push: { productId: "$_id.productId", count: "$count" },
            },
          },
        },
        {
  ...
  async updateCartProducts(
    ...
    if (addProducts) {
      updateData.$push = {
        products: { $each: addProducts.map((id) => new Types.ObjectId(id)) },
      };
    }

    if (removeProducts) {
      updateData.$pull = {
        products: { $in: removeProducts.map((id) => new Types.ObjectId(id)) },
      };
    }
