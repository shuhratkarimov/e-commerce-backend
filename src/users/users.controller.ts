import { Controller, Get, Post, Body, Patch, Param, Delete, Req, Res, UnauthorizedException, Request } from "@nestjs/common";
...
export class UsersController {
  ...
  @ApiResponse({ status: 200, description: 'User logged in and token set in cookie' })
  async login(
  ...
  @ApiResponse({ status: 200, description: 'OTP verified and password updated' })
  ...
  async logout(@Req() req: ExpressRequest, @Res() res: Response): Promise<ResponseMessage> {
    try {
  ...
  @Post("update_profile")
  async updateUser(@Body() updateUserDto: UpdateUserDto, @Request() request): Promise<Object> {
    try {
