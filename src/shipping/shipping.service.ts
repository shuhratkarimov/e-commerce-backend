...
export class ShippingService {
  constructor(@InjectModel(Shipping.name) private shippingModel: Model<Shipping>) {}
  ...
  async update(id: string, updateShippingDto: UpdateShippingDto): Promise<Shipping> {
    const updated = await this.shippingModel.findByIdAndUpdate(id, updateShippingDto, { new: true });
